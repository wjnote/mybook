...
15. 公司项目的整体按钮图标，在框架按钮的基础上再使用 同一的表示
    ```shell
...
16. 在项目中，确认删除类似操作的时候，需要用户确认一次，可以使用下面的代码，  配置参数在 `Popconfirm` 类似
...
18. `up-load` 上传插件的API 如果调用了 `beforeUpload` 并返回false，如果页面有展示已上传的列表，则这次没上传功能的文件还是会展示在已上传列表中，但是文件是没上传的

19. 在项目中如果要获取用户信息(用户信息保存在state里面)，但是只能在顶层框架页里面获取，组件里面的页面是获取不到的，可以使用下面的方法来获取
    ```js
...
<img src="./img/ta404.png" alt="用组件包裹一下就可以了" style="zoom:67%;" />